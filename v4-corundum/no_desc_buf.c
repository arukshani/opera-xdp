...
port_tx_burst_collector(struct port *p, struct burst_tx_collector *b, int free_btx, int wait_all)
{
	...
	for (i = 0; i < n_pkts; i++)
	{
		...
		// printf("length of packet b->len[i]:  %d \n", b->len[i]);
	}
			...
			for (h = 0; h < NUM_OF_PER_DEST_QUEUES; h++)
			{
				if (ring_buff[w][h] != NULL)
				{
					while ((!ringbuf_is_empty(ring_buff[w][h])) && (btx_index < MAX_BURST_TX))
					{
						...
						// printf("len of packet btx2->len[0] : %d \n", btx2->len[0]);
...
thread_func_veth(void *arg)
{
	...
	while (!t->quit)
	{
		...
		for (j = 0; j < n_pkts; j++)
		{
			...
			struct port_params *params = &port_rx->params;

			// printf("&port_rx->params->iface : %s \n", params->iface);
			// printf("nic_iface : %s \n", nic_iface);
...
thread_func_nic(void *arg)
{
	...
	while (!t->quit)
	{
		...
		// printf("receive packets from nic rx n_pkts %d \n", n_pkts);
...
int main(int argc, char **argv)
{
	...
	FILE *file = fopen("/home/dathapathu/emulator/github_code/cr_worker_info.csv", "r");
	if (file)
	{
