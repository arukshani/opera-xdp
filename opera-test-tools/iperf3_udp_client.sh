...
for i in $(seq 0 $num_namespaces); do
    ...
    sudo taskset --cpu-list $cpu_core_id ip netns exec ${myArray[$i]} iperf3 -c ${serverList[$i]} -p $port -t 30 -f g -P 2 -u -l 3400 -b $bandwidth &
    # ip netns exec ${myArray[$i]} iperf3 -c ${serverList[$i]} -p $port -t 30 -f g -P 3 &
