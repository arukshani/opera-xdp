...
static void process_rx_packet(void *data, struct port_params *params, uint32_t len, u64 addr, struct return_process_rx *return_val)
{
    ...
    if (prefix(OUTER_VETH_PREFIX, params->iface))
	{
		...
		if (strcmp(params->iface, "dcout1") == 0) {
			gre_hdr->flags = 0;
		} else if (strcmp(params->iface, "dcout2") == 0) {
			gre_hdr->flags = 1;
		} else if (strcmp(params->iface, "dcout3") == 0) {
			gre_hdr->flags = 2;
		} else if (strcmp(params->iface, "dcout4") == 0) {
			gre_hdr->flags = 3;
		} else if (strcmp(params->iface, "dcout5") == 0) {
			gre_hdr->flags = 4;
		} else if (strcmp(params->iface, "dcout6") == 0) {
			gre_hdr->flags = 5;
		} else if (strcmp(params->iface, "dcout7") == 0) {
			gre_hdr->flags = 6;
